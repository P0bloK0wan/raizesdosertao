...
import { RS_CLUBE, RS_UNIDADES, RS_DIRETORES, RS_CALENDARIO } from "./data.js";
import { getTheme, setTheme, watchGaleria, watchMeta } from "./store.js";
...
/* Barra de progresso da meta (dados em tempo real do Firestore) */
...
/* Monta galeria de fotos/vídeos (pública, tempo real) */
const galeriaEl = document.querySelector("[data-galeria]");
if (galeriaEl) {
  watchGaleria((eventos) => {
    const ordenados = eventos.slice().sort((a, b) => (a.data < b.data ? 1 : -1));
    if (!ordenados.length) {
