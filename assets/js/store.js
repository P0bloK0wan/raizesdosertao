...
export async function reabrirVagaNormal(data, qtd = 1) {
  ...
  await runTransaction(db, async (tx) => {
    ...
    if (vagasFechadas < qtd) {
      throw new Error("Não há vaga normal fechada para reabrir.");
    }
...
export async function removerVagaExtra(data, qtd = 1) {
  const domingoRef = doc(db, "lavajato_domingos", data);
  await runTransaction(db, async (tx) => {
    const domingoSnap = await tx.get(domingoRef);
    const atual = domingoSnap.exists()
...
export function addPastaMidia(nome, fotos = [], link = "") {
  return addDoc(collection(db, "midia"), { nome, fotos, link, tipo: link ? "drive" : "fotos", criadoEm: serverTimestamp() });
}
