...
import { collection, onSnapshot, where, query } from "https://www.gstatic.com/firebasejs/10.13.0/firebase-firestore.js";
...
if(alvo){
 ...
 onSnapshot(query(collection(db,"eventosPublicos"),where("publicado","==",true)),snap=>{
   const eventos=snap.docs.map(d=>d.data()).filter(e=>e.publicado===true&&typeof e.data==="string"&&e.data>=hoje).sort((a,b)=>a.data.localeCompare(b.data)).slice(0,30);
