...
test("mídia carrega pastas antigas e mostra erros",()=>{const store=read("assets/js/store.js");const panel=read("assets/js/painel-lideranca.js");assert.match(store,/onSnapshot\(collection\(db, "midia"\)/);assert.match(panel,/id="midia-erro"|getElementById\("midia-erro"\)/);assert.match(read("painel-lideranca.html"),/id="midia-erro"/)});
test("clientes e prospects não aparecem no painel da liderança",()=>{const html=read("painel-lideranca.html");assert.doesNotMatch(html,/id="prospects"|prospects\\.js|Clientes e prospects/)});
test("painéis preservam autenticação e áreas principais",()=>{const lideranca=read("assets/js/painel-lideranca.js");const unidade=read("assets/js/painel-unidade.js");assert.match(lideranca,/exigirSessao\("lideranca"/);assert.match(unidade,/exigirSessao\("unidade"/);for(const id of ["midia","lavajato","mural-avisos"])assert.match(read("painel-lideranca.html"),new RegExp('id="'+id+'"'))});
