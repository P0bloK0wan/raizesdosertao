...
**"Já corrigi/atualizei o site (fiz o push), mas ele continua parecendo com a versão antiga — um botão que devia ter sumido ainda aparece, uma tela parece diferente do que devia."**
→ O site guarda uma cópia local das páginas no navegador de quem visita (pra abrir mais rápido e funcionar sem internet), usando um "Service Worker" (arquivo [`sw.js`](./sw.js)). Se essa cópia ficar desatualizada, o navegador da pessoa pode continuar mostrando a versão antiga por um bom tempo, mesmo depois do GitHub Pages já estar com o código novo no ar. Pra forçar a atualização:
  - No celular ou computador, feche a aba/o site completamente e abra de novo (às vezes já resolve).
  - Se não resolver: nas Ferramentas do desenvolvedor (F12) → aba **Application** (ou **Aplicativo**) → **Service Workers** → **Unregister**; e em **Storage**/**Armazenamento** → **Clear site data**/**Limpar dados do site**. Depois recarregue a página.
  - Como desenvolvedor: sempre que mudar algum arquivo do "app shell" listado em `sw.js`, aumente o número em `CACHE_NAME` (ex.: de `"raizes-do-sertao-v6"` pra `"raizes-do-sertao-v7"`) — isso avisa o navegador de todo mundo que a versão mudou e força a atualização automaticamente, sem precisar que cada pessoa limpe o cache na mão.
